...
import com.teach.teach1907.design.AutoLineFeedLayout;
import com.teach.teach1907.design.RatingBar;
...
public class CourseDescribesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public CourseDescribesAdapter(Context pContext, List<LessonComment> pCommentList, CourseDetailInfo pCourseInfo) {
        mContext = pContext;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == i) {
            View inflate = LayoutInflater.from(mContext).inflate(R.layout.course_title, null);
            return new ViewHolderT(inflate);
        } else {
            return new ViewHolder(LayoutInflater.from(mContext).inflate(R.layout.course_desc_adapter_layout, null));
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (itemViewType == i) {
            ...
            viewHolderT.tvDiscussNum.setText(courseInfo.getStudentnum()+"人学习");
            viewHolderT.tvDiscussRate.setText(courseInfo.getRate()+"好评率");
        } else {
            ViewHolder viewHolder= (ViewHolder) holder;
            LessonComment comment = commentList.get(position-1);
            GlideUtil.loadImage(viewHolder.avater, comment.getImg());
            viewHolder.name.setText(comment.getUsername());
            viewHolder.time.setText(TimeUtil.parseTimeFormat(Long.parseLong(comment.getAdd_time() + ""), "yyyy-MM-dd"));
            viewHolder.ratingBar.setStar(StringUtils.getRoundRank(comment.getRank()));
            if (comment.getTags() != null && comment.getTags().size() > 0) {
                viewHolder.aflContainer.setVisibility(View.VISIBLE);
                viewHolder.aflContainer.removeAllViews();
                for (String tag : comment.getTags()) {
                    View child = LayoutInflater.from(mContext).inflate(R.layout.item_comment_tag, viewHolder.aflContainer, false);
                    TextView view = child.findViewById(R.id.tv_comment_tag);
                    view.setText(tag);
                    viewHolder.aflContainer.addView(child);
                }
            }
            viewHolder.content.setText(SpanUtils.handlerEmotion(mContext, comment.getContent()));
            if (comment.getReplycontent() != null && comment.getReplycontent().size() > 0) {
                ...
                viewHolder.replyContent.setText(spannable);
            } else viewHolder.replyContent.setVisibility(View.GONE);
        }
    }
